 ...
 */
const { otpTemplate, passwordResetTemplate } = require("./templates");
...
/* ── Convenience wrappers — what the controllers actually import ───── */

/**
 * OTP email (registration or password reset).
 * This is the function `otp.controller.js` and `password.controller.js`
 * import — it was missing before, causing "sendOtpEmail is not a function".
 */
const sendOtpEmail = ({ to, otp, expiresMinutes = 10 }) =>
  sendMail({
    to,
    subject: "Your OTP Code — APV Library",
    html: otpTemplate(otp, { expiresMin: expiresMinutes }),
    text: `APV Library — Your OTP is: ${otp}\nExpires in ${expiresMinutes} minutes.`,
  });

/**
 * Password reset OTP email — same OTP mechanism, different subject/copy.
 */
const sendPasswordResetEmail = ({ to, otp, expiresMinutes = 10 }) =>
  sendMail({
    to,
    subject: "Password Reset OTP — APV Library",
